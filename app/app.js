...
async function handleCreateIssue() {
    ...
    const errorType = lastAnalysisResult.parsed.errorType || 'error';
    const coreMsg = (lastAnalysisResult.parsed.coreMessage || '').substring(0, 60);
    const title = `[Bug] ${errorType}: ${coreMsg}`.replace(/[^\x20-\x7E]/g, '').trim();
...
async function handleCreatePR() {
    ...
    try {
        // Generate branch name (ASCII-safe)
        const timestamp = Date.now();
        const safeErrorType = (lastAnalysisResult.parsed.errorType || 'error').replace(/[^a-zA-Z0-9_-]/g, '-');
        const branchName = `fix/${safeErrorType}-${timestamp}`;
        
        let branchCreated = false;
        
        // If we have actual code context, create a branch with the real file fix
        if (lastAnalysisResult.fix.hasCodeContext && lastAnalysisResult.fix.sourceFile) {
            ...
            if (originalContent && lastAnalysisResult.fix.before) {
                const fixedContent = originalContent.replace(
                    lastAnalysisResult.fix.before,
                    lastAnalysisResult.fix.after
                );
                
                const commitMessage = `fix: ${safeErrorType} in ${filePath}\n\nAuto-generated fix by Error Debugger\nRoot cause: ${(lastAnalysisResult.rootCause.rootCause || '').substring(0, 200)}`;
                ...
                if (branchResult.success) {
                    ...
                    console.warn('Code fix branch failed, will create description-only branch:', branchResult.error);
                }
            }
        }
        
        // If no code fix was applied, create a branch with a fix description file
        // GitHub PRs REQUIRE a branch that differs from the base branch
        if (!branchCreated) {
            const fixDescription = [
                `# Fix: ${lastAnalysisResult.parsed.errorType || 'Error'}`,
                '',
                `## Root Cause`,
                lastAnalysisResult.rootCause.rootCause || 'See PR description for details.',
                '',
                `## Solution`,
                lastAnalysisResult.rootCause.solution || lastAnalysisResult.fix.explanation || 'See PR description for details.',
                '',
                `## Suggested Code Change`,
                '',
                '### Before',
                '```',
                lastAnalysisResult.fix.before || '(no code pattern identified)',
                '```',
                '',
                '### After',
                '```',
                lastAnalysisResult.fix.after || '(see solution above)',
                '```',
                '',
                `---`,
                `*Auto-generated by Error Debugger*`,
            ].join('\n');
            
            const fixFilePath = `.github/fixes/${safeErrorType}-${timestamp}.md`;
            const commitMessage = `fix: add fix description for ${safeErrorType}`;
            
            const branchResult = await createFixBranch(branchName, fixFilePath, fixDescription, commitMessage);
            
            if (!branchResult.success) {
                if (statusEl) statusEl.innerHTML = `<span class="error">Branch creation failed: ${branchResult.error}</span>`;
                if (btn) btn.disabled = false;
                return;
            }
        }
        
        // Create the PR (branch now guaranteed to exist with at least one commit)
        const title = `Fix: ${safeErrorType} - ${(lastAnalysisResult.parsed.coreMessage || '').substring(0, 50)}`.replace(/[^\x20-\x7E]/g, '').trim();
        ...
        if (result.success) {
            ...
            if (statusEl) statusEl.innerHTML = `<span class="error">PR failed: ${result.error}</span>`;
            if (btn) btn.disabled = false;
        }
    } catch (e) {
